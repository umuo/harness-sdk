...
An Agent Skill is a directory containing a `SKILL.md` file. Register one Skill
or recursively discover a directory:
...
The initial system prompt contains only Skill names, descriptions and
locations. When a Skill is relevant, the model calls the automatically
registered `skill_load` Tool. Omitting `resource` loads the Markdown body of
`SKILL.md`; supplying a relative resource path loads that UTF-8 text file from
inside the Skill root. Skill instructions and references therefore consume
context only after activation.

`skillsFrom` fails fast if any discovered `SKILL.md` is invalid. Applications
that want partial discovery can inspect `SkillLoader.discover(path)` and decide
how to handle its diagnostics. See [Agent Skills](skills.md) for the file
format, boundaries and security behavior.
